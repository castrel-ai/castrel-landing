...
Castrel Proxy 是一个轻量级的 **本地代理客户端**。它会通过 WebSocket 连接到 Castrel，接收来自服务器的指令，在你的机器上执行它们，再把结果返回给服务器。
...
安装脚本会从 Castrel 的公开包存储下载对应平台二进制，校验 `.sha256` 后安装 `castrel-proxy`。如需使用私有镜像，可在运行前设置 `CASTREL_PROXY_PACKAGE_BASE_URL`。

默认会安装到用户级目录 `~/.local/bin`（不需要 sudo）。
...
如果在交互式终端里且未传 `--install-dir`，安装脚本会提示你直接填写安装目录；留空则使用默认目录 `~/.local/bin`。
...
如果安装目录不在 `$PATH` 中，脚本会输出可直接添加到 shell 配置文件的命令。

当你使用 `--install-dir` 时，脚本也会检查 PATH，并给出明确提示，确保新终端能直接找到该命令。
...
- 把该 Connector 绑定到已配对的 Castrel Proxy 节点
- 在会话里使用该 Connector 查询时，请求会通过 Proxy 的 `http_proxy_call` 在节点本地转发到内网端点

6. 通过 Connector/Castrel Proxy 节点执行本地命令（例如 `free` / `top`）
