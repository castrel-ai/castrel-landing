<script setup lang="ts">
    ...
    const { data: page } = await useAsyncData('feishu-privacy', async () => {
        const allDocs = await queryCollection('docs').all();
        const targetPath = '/feishu/privacy-policy';
        ...
        if (!found && process.dev) {
            ...
            console.log('Available feishu paths:', allDocs?.filter(doc => doc.path?.startsWith('/feishu')).map(d => d.path));
        }
