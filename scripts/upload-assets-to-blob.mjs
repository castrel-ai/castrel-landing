...
function loadEnvFile(path, options = {}) {
    ...
    for (const line of content.split(/\r?\n/)) {
        ...
        const key = trimmed.slice(0, equalIndex).trim().replace(/^export\s+/, '')
        const value = stripQuotes(trimmed.slice(equalIndex + 1))
        if (!key) continue

        if (process.env[key] === undefined || (override && !lockedEnvKeys.has(key))) {
...
async function main() {
    ...
    for (const dirName of supportedDirs) {
        ...
        const exists = await fs.stat(dirPath).then((s) => s.isDirectory()).catch(() => false)
        if (!exists) continue
