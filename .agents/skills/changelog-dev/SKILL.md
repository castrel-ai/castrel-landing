...
- 完整发版清单是本次发版的**全集索引**：覆盖所有 Issue，包括已拥有独立章节的核心功能，每个都转化为极简的一句话动宾结构（核心功能的清单条目精简概括即可，详情见正文章节）。
...
4. **主次分明原则 (Core Rule)**：
   ...
   - **清单为全集**：`## 完整发版清单` / `## Full Release Checklist` 必须覆盖本次发版的**所有** Issue（含已有独立章节的核心功能）。核心功能在清单中用一句话精简概括，详情仍由正文章节承载；其余常规新功能、流程优化、体验升级只出现在清单中，绝不在正文里作详细段落介绍。
